...
// login টেবিলের সাথে JOIN করে মালিকের নাম এবং সময় আনা
$result = mysqli_query($mysqli, "SELECT lost.*, login.name as owner_name FROM lost JOIN login ON lost.login_id = login.id ORDER BY lost.id DESC");
?>
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <title>Lost Items | UIU RecycleBin</title>
    ...
    <style>
        * { margin:0; padding:0; box-sizing:border-box; font-family:'Poppins', sans-serif; }
        ...
        .nav-links a { text-decoration: none; color: #555; font-size: 14px; font-weight: 500; transition: 0.3s; }
        .nav-links a:hover, .nav-links a.active { color: #e74c3c; font-weight: 600; }

        .header { display:flex; justify-content:space-between; align-items:center; margin-bottom:30px; margin-top: 20px; flex-wrap: wrap; }
        .btn-add { background:#e74c3c; color:white; padding:10px 20px; text-decoration:none; border-radius:8px; font-weight:600; }
        
        /* Card Design */
        .card-container { display: grid; grid-template-columns: repeat(auto-fill, minmax(280px, 1fr)); gap: 25px; margin-bottom: 40px; }
        .item-card { background:#fff; padding:25px; border-radius:15px; box-shadow:0 4px 15px rgba(0,0,0,0.05); border-top:5px solid #e74c3c; }
        ...
        @media (max-width: 768px) { .nav-links { display: none; } .navbar { padding: 15px 5%; } body { padding-left: 5%; padding-right: 5%; } }
    </style>
</head>
<body>
...
<div class="header">
    <h2><i class="fa-solid fa-magnifying-glass" style="color:#e74c3c;"></i> Lost Items</h2>
    <a href="addLost.php" class="btn-add">Report Lost</a>
</div>
            ...
            <div class="action-links">
                <div>
                    <?php if(isset($_SESSION['id']) && $res['login_id'] == $_SESSION['id']): ?>
                        <a href="editLost.php?id=<?php echo $res['id']; ?>" style="color:#3498db;"><i class="fa-solid fa-edit"></i></a>
                        <a href="deleteLost.php?id=<?php echo $res['id']; ?>" style="color:#e74c3c; margin-left:10px;" onclick="return confirm('Delete?')"><i class="fa-solid fa-trash"></i></a>
                    <?php endif; ?>
                </div>
                <a href="post_details.php?id=<?php echo $res['id']; ?>&type=lost" style="text-decoration:none; color:#27ae60; font-weight:600;">It's Mine!</a>
            </div>
        </div>
    <?php } ?>
</div>
